...
Classify positive and negative customer-review sentiment from review text.
...
- **Task:** Classification
- **Target:** `sentiment derived from Score`
- **Primary metric:** Macro F1
- **Goal:** Classify positive and negative customer-review sentiment from review text.

## Dataset

- **Availability:** Download required: place `Reviews.csv` in this directory
- **Recorded source:** The original notebook expects the Amazon Fine Food Reviews file named Reviews.csv; the source link was not recorded in the repository.
- **Target:** `sentiment derived from Score`
...
- Three-star neutral reviews are excluded to keep the binary objective explicit.
- TF-IDF is fitted inside the pipeline to prevent vocabulary leakage.
...
```text
├── customer_review_sentiment_classification.ipynb
└── README.md
```
...
jupyter lab 'customer_review_sentiment_classification.ipynb'
```
