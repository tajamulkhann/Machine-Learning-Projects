...
Classify diagnostic measurements as malignant or benign while keeping preprocessing isolated from the test set.
...
- **Goal:** Classify diagnostic measurements as malignant or benign while keeping preprocessing isolated from the test set.

## Dataset

- **Availability:** Included: `Breast Cancer dataset.csv`
...
- Identifiers and the empty export column are removed.
- Scaling is performed inside each model pipeline, never on the full dataset.
...
|---|---:|---:|
| Logistic Regression | 0.9649 | 0.9619 |
...
jupyter lab 'breast_cancer_classification.ipynb'
```
